...
#                     assert len(s_batch_agent) == FLAGS.num_iter, \
    ...
    def agent(self,agent_id, config, game,network, tm_subset, model_weights_queue, experience_queue):
...
#                                             tm_subsets = np.array_split(game.wk_indexes, FLAGS.num_agents)
        tm_subsets = np.array_split(game.wk_indexes, self.num_agents)
...
#                                             for i in range(FLAGS.num_agents):
        for i in range(self.num_agents):
            agents.append(mp.Process(target=self.agent, args=(i, config, game, network,tm_subsets[i], model_weights_queues[i], experience_queues[i])))
